define("overviewer", ["require", "exports", "bloom-filters"], function (require, exports, bloom_filters_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    let filter = new bloom_filters_1.BloomFilter(10, 4);
    // insert data
    filter.add('alice');
    filter.add('bob');
    var overviewer = {};
    ...
    overviewer.util = {
        ...
        'initialize': function () {
            ...
            overviewer.control = L.Control.extend({
                ...
                onChange: function (ev) {
                    ...
                    overviewer.layerCtrl = L.control.layers(base_layers, overlay_layers, { collapsed: false })
            ...
            overviewer.map.attributionControl.setPrefix('<a href="https://overviewer.org">Overviewer/Leaflet</a>');
            overviewer.map.on('baselayerchange', function (ev) {
                ...
                if (overviewer.current_layer[overviewer.current_world]) {
                    ...
                    const currentWorldCoords = overviewer.util.fromLatLngToWorld(center.lat, center.lng, overviewer.current_layer[overviewer.current_world].tileSetConfig);
                    const newMapCoords = overviewer.util.fromWorldToLatLng(currentWorldCoords.x, currentWorldCoords.y, currentWorldCoords.z, ev.layer.tileSetConfig);
            ...
            overviewer.layerCtrl = L.control.layers(overviewer.collections.mapTypes[overviewerConfig.worlds[0]], overviewer.collections.overlays[overviewerConfig.worlds[0]], { collapsed: false })
                .addTo(overviewer.map);
        ...
        'runReadyQueue': function () {
            if (overviewer.util.readyQueue.length === 0)
                return;
        ...
        'updateHash': function () {
            ...
            var ovconf = currTileset.tileSetConfig;
            var coordinates = overviewer.util.fromLatLngToWorld(overviewer.map.getCenter().lat, overviewer.map.getCenter().lng, ovconf);
        ...
        'goToHash': function () {
            ...
            var latlngcoords = overviewer.util.fromWorldToLatLng(parseInt(coords[1]), parseInt(coords[2]), parseInt(coords[3]), ovconf);
            if (zoom == 'max') {
                zoom = ovconf.maxZoom;
            }
            else if (zoom == 'min') {
